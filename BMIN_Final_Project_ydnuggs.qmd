...
The ADI is useful for health disparities research, which is an important area in all fields of research. There are several other indices that measure socioeconomic status (SES) in some way; these include the Townsend Deprivation Index, Social Disadvantage Index, and others. The ADI is one of the oldest validated indices for SES, and therefore it is the one we use in this paper. Future research can include doing similar analyses for other commonly used SES indices.

## Methods {#sec-methods}

In order to calculate the ADI for Pennsylvania in 2020, we first analyze the 17 sociodemographic factors that Singh described in his paper. We import these from the Decennial Census and American Community Survey. We visualize each characteristic and log-transform those without a Normal distribution. We then use factor analysis to calculate the ADI. We import health outcomes data from PLACES. Lastly, we test whether the ADI is associated with health outcomes data.
...
Next we examine the distributions of the health outcomes data, using both bar graphs and chloropleth maps to see the distrbutions across the state of Pennsylvania.
...
Next we create chloropleth maps for each of the health outcomes variables. These maps help us visualize the distribution of the health outcomes data across Pennsylvania tracts.
...
We can compare our factor scores to those in Singh's paper (see @sec-methods). In general the characteristics do seem to have similar factor scores in similar directions, but of lower magnitude than in Singh's paper.
...
In this project, we analyzed the 17 characteristics that Singh used to create the ADI. Whereas he used data from across the United States of America between the years 1969 and 1998, we used data from Pennsylvania 2016-2020. In this way we analyzed whether the 17 characteristics he used are still useful in our population. The factor scores are similar but of lower magnitude, and the ADI we devised is correlated with health outcomes including rates of disability, coronary heart disease, obesity, stroke, and self-reported health.

It seems that the ADI devised by Singh remains useful in our population; however, it is important to use updated data for our patient population, as our factor scores using 2020 Pennsylvania data differ from Singh's factor scores. Furthermore, the weaker factor score magnitudes indicate that these might not be the best characteristics, of all those available, to include in an ADI. There are many additional characteristics that can be considered for inclusion, either as an addition or replacement; some are included in the Supplements section.
...
-   To investigate other sociodemographic characteristics that may be useful in an ADI, and consider whether these should be in addition to or instead of some of the characteristics currently in the ADI

-   To re-examine Singh's ADI using 2020 data for the entire country, and state-by-state, and test whether the measuresments do differ significantly by state; in this way, we can decide whether there should be different ADIs for each state or whether it is sufficient to have one country-wide

Overall, our ADI is a useful measure for health disparities research in Pennsylvania in the 2020s.
