---
title: "Area Deprivation Index for Pennsylvania 2020"
...
In 2002, Gopal Singh published the Area Deprivation Index (ADI), a “composite area-based deprivation index for the United States” based on census data from 1969 to 1998. It has become a tool used to examine, on an area-based scale, how social determinants of health can have an impact on health outcomes. The Area Deprivation Index uses factor score coefficients based on census data. However, neighborhoods and their sociodemographic characteristics can change over time; therefore, it is important to re-examine these calculations intermittently. Furthermore, associations can differ by states, where there are different laws and demographic factors affecting access to resources. In this project, we use recent data from the 2020 U.S. decennial census and the 2016-2020 5-year American Community Survey (ACS) to reconstruct the ADI for Pennsylvania and re-calculate the factor score coefficients. We then test whether the ADI is associated with several health outcomes, using tract level data.

The ADI is useful for health disparities research, which is an important area in all fields of research. There are several other indices that measure socioeconomic status (SES) in some way; these include the Townsend Deprivation Index, Social Disadvantage Index, and others. The ADI is one of the oldest validated indices for SES, and therefore it is the one we use in this paper.

## Methods {#sec-methods}

In order to calculate the ADI for Pennsylvania in 2020, we first analyze the 17 sociodemographic factors that Singh described in his paper. We import these from the 2020 U.S. Decennial Census Survey and 2016-2020 5-Year American Community Survey. We visualize each characteristic and log-transform those without a Normal distribution. We then use factor analysis to derive factor-score coefficients with which we can calculate the ADI for each tract.

We import health outcomes data from 2020 CDC PLACES, and use these to test whether the ADI is associated with health outcomes data. We chose five health outcomes to test: obesity because it is associated with many health outcomes; stroke and coronary heart disease because they can lead to significant morbidity and mortality; and disability and self-reported health rating because they can serve as quality of life metrics.
...
### Preparing the data: predictors, i.e. sociodemographic variables

These are the factors from Singh's seminal paper, and their corresponding factor score coefficients, that we seek to replicate in 2020 Pennsylvania (as listed in PMID 12835199).

```{r Singh_factors}
# Create the data frame with the characteristics and corresponding factor score coefficients
data_singh <- data.frame(
  Characteristic = c(
  ...
  Factor_Score_Coefficient = c(
    0.0849, -0.0970, -0.0874, -0.0977, 0.0936, -0.0688, -0.0781, 
    -0.0770, -0.0615, 0.0806, 0.0977, 0.1037, 0.0719, 0.0694, 
    0.0877, 0.0510, 0.0556
  )
)

# View the table
data_singh |>
  kable(format = "html", col.names = c("Census Block Characteristics", "Factor Score Coefficients")) |>
  kable_styling(
...
We create a dataset with the 2020 Decennial Census variables from the Demographic and Housing Characteristics (DHC) dataset. We do the same for the 2016-2020 ACS 5-year Variables. These datasets can be used to identify the variables of interest based on their decriptions in the files.
...
## Population aged ≥ 25 y who did not graduate high school
...
### Preparing the data: health outcomes
...
Create a dataset of several important health outcomes. Of note, mortality data is not included, as it is not available via PLACES, and special permission is required for access. The health outcomes data are expressed as frequencies.
...
### Preparing the data: merging the predictor and outcomes datasets
...
Next we check our data missingness. Overall we have fairly low missingness in the sociodemographic characteristics, and no missingness in the health outcomes variables.
...
# Reset row names to avoid duplication issues in the final table
...
missingness_table_demographics |>
  kable(col.names = c("Variable", "Missing Data"),
        caption = "Missing Data for Demographic Variables (%)",
        align = c("l", "r"),
        format = "markdown")
...
missingness_table_outcomes |>
  kable(col.names = c("Variable", "Missing Data"),
        caption = "Missing Data for Health Outcomes Variables (%)",
        align = c("l", "r"),
        format = "markdown")
```
...
### Examination of the data: sociodemographic variables and health outcomes
...
Next we examine the distributions of the health outcomes data, using both bar graphs and chloropleth maps to see the distributions across the state of Pennsylvania in 2020.
...
Next we create chloropleth maps for each of the health outcomes variables. These maps help us visualize the distribution of the health outcomes data across Pennsylvania tracts. The white areas indicate missing data.
...
### Creation of the ADI for 2020 Pennsylvania

We perform factor analysis to reconstruct the ADI for 2020 Pennsylvania.
...
factor_scores <- factor.scores(merged_data_subset_clean[,2:18], fa_result) #Each row corresponds to a geographic tract, and each column corresponds to a factor score coefficient, based on the factors extracted in the factor analysis.
```

We present the data in a table similar to the one for Singh's data above, in order to facilitate comparison.

```{r}
# Create the data frame with the characteristics and corresponding factor score coefficients
data_ours <- data.frame(
  Characteristic = c(
    "Percent of the block group’s population aged ≥ 25 years with < 9 years of education",
    "Percent aged ≥ 25 years with an Associate's diploma or higher degree",
    "Percent of employed persons ≥16 years of age in white-collar occupations",
    "Median family income",
    "Income disparity measured by Gini index",
    "Median home value",
    "Median gross rent",
    "Median monthly owner costs",
    "Percent owner-occupied housing units (home ownership rate)",
    "Percent of civilian labor force population ≥ 16 years of age unemployed (unemployment rate), log",
    "Percent of families below the poverty level, log",
    "Percent of population below 150% of the poverty threshold, log",
    "Percent of single-parent households with children < 18 years of age, log",
    "Percent of occupied housing units without a motor vehicle, log",
    "Percent of occupied housing units without a telephone, log",
    "Percent of occupied housing units without complete plumbing, log",
    "Percent of occupied housing units with more than one person per room (crowding), log"
  ),
  Factor_Score_Coefficient = c(
    -0.624, 0.776, 0.792, 0.885, -0.176, 0.769, 0.627, 0.405, 0.488,
-0.338, -0.805, -0.888, -0.670, -0.420, -0.265, -0.166, -0.294
  )
)
...
We can compare our factor scores to those in Singh's paper (see @sec-methods).
...
Plot a map of the ADI in Pennsylvania tracts. We see the distribution of ADI across Pennsylvania; it appears there are regions of higher ADI around the Philadelphia and Pittsburgh areas.
...
### Test of association between ADI and health outcomes

We test whether the ADI is associated with the five health outcomes of interest.
...
There do seem to be linear associations, although the association between ADI and self-reported health seems a little less linear than the others, and the association between ADI and coronary heart disease seems weaker than for the other variables.

We therefore quantify the associations using the Pearson correlation.
...
We also use linear regression to test whether ADI is significantly associated with the five health outcomes of interest.

```{r}
summary(lm(Rate_Disability ~ ADI, data = merged_data_all))
summary(lm(Rate_Healthrating ~ ADI, data = merged_data_all))
summary(lm(Rate_Heart ~ ADI, data = merged_data_all))
summary(lm(Rate_Obesity ~ ADI, data = merged_data_all))
summary(lm(Rate_Stroke ~ ADI, data = merged_data_all))
```

In simple linear regression models, ADI is significantly associated with rates of disability ($\beta$ = -5.8, p\<2e-16), coronary heart disease ($\beta$ = -0.7, p\<2e-16), obesity ($\beta$ = -4.2, p\<2e-16), stroke ($\beta$ = -0.8, p\<2e-16), and self-reported health ($\beta$ = -5.6, p\<2e-16).

## Conclusion

In this project, we analyzed the 17 characteristics that Singh used to create the ADI. Whereas he used data from across the United States of America between the years 1969 and 1998, we used data from Pennsylvania 2016-2020. In this way we analyzed whether the 17 characteristics he used are still useful in our population. We successfully replicated Singh's methods to create an ADI using factor score analysis for Pennsylvania 2020. We then demonstrated that the ADI we devised is significantly correlated with health outcomes including rates of disability, coronary heart disease, obesity, stroke, and self-reported health. Therefore, our ADI appears to be a useful measure for health disparities research in Pennsylvania in the 2020s.

Data missingness, though low, is a limitation of our analyses. In addition, for some of the sociodemographic variables, it is impossible to use the exact variable that Singh used in our modern analyses; for example, income disparity cannot be measured in the exact same way several decades later.

There are many potential future directions for this research.

-   To compare the associations between Singh's ADI and our ADI with health outcomes measures

-   To test the association between our ADI and mortality, and other important health outcomes measures

-   To investigate other sociodemographic characteristics that may be useful in an ADI, and consider whether these should be in addition to or instead of characteristics currently in the ADI; some additional characteristics to consider are included in the Supplements section.

-   To re-examine the ADI using 2020 data for the entire country, and state-by-state, and test whether the measurements do differ significantly by state; in this way, we can decide whether there should be different ADIs for each state or whether it is sufficient to have one country-wide

Overall, our ADI is a useful measure for health disparities research in Pennsylvania in the 2020s, and significantly associated with important health outcomes measures.
...
These are some additional variables that may be of interest to study and potentially incorporate in future ADIs.
