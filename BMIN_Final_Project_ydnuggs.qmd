...
merged_data_subset <- merged_data[, vars_of_interest]
...
rownames(missingness_table) <- NULL
...
# Subset the dataframe to include only the columns you're interested in
merged_data_subset <- merged_outcomes[, vars_outcomes]
...
``` t
First, we examine the distributions of the health outcomes data.
```
...
plot_obesity = ggplot(data = merged_data_outcomes) +
  ...
  labs(title = "Obesity Percentage by Census Tract",
       fill = "Obesity %") +
  theme_minimal() +
  theme(legend.position = "bottom")

plot_stroke = ggplot(data = merged_data_outcomes) +
  geom_sf(aes(fill = Rate_Stroke), color = "white", size = 0.2) + 
  scale_fill_viridis_c(option = "C") +
  labs(title = "Stroke Percentage by Census Tract",
       fill = "Stroke %") +
  theme_minimal() +
  theme(legend.position = "bottom")

plot_heart = ggplot(data = merged_data_outcomes) +
  geom_sf(aes(fill = Rate_Heart), color = "white", size = 0.2) + 
  scale_fill_viridis_c(option = "C") +
  labs(title = "Coronary Heart Disease Percentage by Census Tract",
       fill = "Coronary Heart Disease %") +
  theme_minimal() +
  theme(legend.position = "bottom")

plot_disability = ggplot(data = merged_data_outcomes) +
  geom_sf(aes(fill = Rate_Disability), color = "white", size = 0.2) + 
  scale_fill_viridis_c(option = "C") +
  labs(title = "Disability Percentage by Census Tract",
       fill = "Disability %") +
  theme_minimal() +
  theme(legend.position = "bottom")

plot_healthrating = ggplot(data = merged_data_outcomes) +
  geom_sf(aes(fill = Rate_Healthrating), color = "white", size = 0.2) + 
  scale_fill_viridis_c(option = "C") +
  labs(title = "Health Rating Average by Census Tract",
       fill = "Health Rating %") +
  theme_minimal() +
  theme(legend.position = "bottom")
...
These are some additional calculations and graphicst that were used in the data preparation and cleaning.

``` t
This is summary information for the variables of interest.
```
...
data_places_obesity |> select(Data_Value) |> tbl_summary()
data_places_stroke |> select(Data_Value) |> tbl_summary()
data_places_heart |> select(Data_Value) |> tbl_summary()
data_places_disability |> select(Data_Value) |> tbl_summary()
data_places_healthrating |> select(Data_Value) |> tbl_summary()
```
